...
WebSocketFrame.prototype.addData = function(buffers, fragmentationType) {
    var temp;
    if (this.parseState === DECODE_HEADER) {
        if (buffers.length >= 2) {
            buffers.splice(0, 2).copy(this.frameHeader, 0);
    ...
    if (this.parseState === WAITING_FOR_16_BIT_LENGTH) {
        if (buffers.length >= 2) {
            buffers.splice(0, 2).copy(this.frameHeader, 2);
        ...
        if (buffers.length >= 8) {
            buffers.splice(0, 8).copy(this.frameHeader, 2);
    ...
    if (this.parseState === WAITING_FOR_MASK_KEY) {
        if (this.mask) {
            if (buffers.length >= 4) {
                buffers.splice(0, 4).copy(this.frameHeader, 0);
                this.maskPos = 0;
    ...
    if (this.parseState === WAITING_FOR_PAYLOAD) {
        ...
        if (buffers.length >= this.length) {
            this.binaryPayload = buffers.splice(0, this.length);
            if (this.mask) {
...
WebSocketFrame.prototype.throwAwayPayload = function(buffers) {
    if (buffers.length >= this.length) {
        buffers.splice(0, this.length);
        this.parseState = COMPLETE;
        return true;
    }
    return false;
...
WebSocketFrame.prototype.toBuffer = function(nullMask) {
    ...
    if (this.opcode === 0x08) {
        ...
        if (this.binaryPayload) {
            length += this.binaryPayload.length;
        }
    ...
    output = new Buffer(length + headerLength + (this.mask ? 4 : 0));
    ...
    if (length > 0xFFFF) {
        // write 64-bit length
        ctio.wuint64([0x00000000, length], 'big', output, outputPos);
        outputPos += 8;
    }
    else if (length > 125) {
        // write 16-bit length
        ctio.wuint16(length, 'big', output, outputPos);
        outputPos += 2;
    }
