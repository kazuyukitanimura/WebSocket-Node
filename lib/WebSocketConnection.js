...
function WebSocketConnection(socket, extensions, protocol, maskOutgoingPackets, config) {
    ...
    // the Buffers will handle the data streaming in
...
WebSocketConnection.prototype.handleSocketData = function(data) {
    ...
    // Add received data to our buffers, which efficiently holds received
    ...
    while (this.connected && this.currentFrame.addData(this.buffers)) {
...
WebSocketConnection.prototype.fragmentAndSend = function(frame) {
    ...
    var threshold     = config.fragmentationThreshold;
    var binaryPayload = frame.binaryPayload;
    var length        = binaryPayload.length;
    
    if (config.fragmentOutgoingMessages && binaryPayload && length > threshold) {
        var maskBytes     = this.maskBytes;
        var frameHeader   = this.frameHeader;

        var currentFrame = new WebSocketFrame(maskBytes, frameHeader, config);
        
        currentFrame.opcode = frame.opcode;
        
        // fin set on last frame only
        currentFrame.fin = false;
        
        // Slice the right portion of the original payload
        var sliceStart = 0;
        var sliceEnd   = threshold;
        currentFrame.binaryPayload = binaryPayload.slice(0, sliceEnd);
        
        this.sendFrame(currentFrame);

        for (var i = Math.ceil(length / threshold) - 2; i--;) {
            currentFrame = new WebSocketFrame(maskBytes, frameHeader, config);
            
            // continuation opcode
            currentFrame.opcode = 0x00;
            ...
            sliceStart = sliceEnd;
            sliceEnd  += threshold;
            currentFrame.binaryPayload = binaryPayload.slice(sliceStart, sliceEnd);
        ...
        // fin set
        currentFrame.fin = true;
        
        // Slice the right portion of the original payload
        currentFrame.binaryPayload = binaryPayload.slice(sliceEnd, length);
